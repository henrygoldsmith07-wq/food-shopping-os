  ...
  return (Array.isArray(items) ? items : []).map((item) => {
    ...
    if (row && row.count >= 2) {
      ...
      return {
        ...
        wasteNote: note,
        // The structural record of this automatic change: what it was,
        // what it became, and the evidence behind it — the adaptation
        // explainer reads this instead of re-deriving the story.
        autoReduction: {
          reason: 'binned',
          fromQty: item.qty,
          toQty: reduced || item.qty,
          applied: Boolean(reduced),
          count: row.count,
      ...
      if (adjusted) {
        return {
          ...
          autoReduction: {
            ...
            toQty: adjusted.qty,
            applied: true,
            recipeName: scrap.recipeName,
            discards: scrap.discards,
            cooks: scrap.cooks,
            lastBinnedAt: scrap.lastDate,
          },
