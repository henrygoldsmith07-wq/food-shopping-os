...
import PantryAddForm from './PantryAddForm.jsx';
import KitchenInventory from './KitchenInventory.jsx';
  ...
  return (
    <div className="px-5 pb-8 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2.5">
        <button
          onClick={() => { setAdding((v) => !v); setCapturing(false); setScanning(false); setStocking(false); }}
        ...
        <button
          onClick={() => { setCapturing((v) => !v); setAdding(false); setScanning(false); setStocking(false); }}
          className="press rounded-2xl border py-3 text-[0.84375rem] font-extrabold"
      ...
      <button
        onClick={() => { setScanning((v) => !v); setAdding(false); setCapturing(false); setStocking(false); }}
      ...
      <button
        onClick={() => { setStocking((v) => !v); setAdding(false); setCapturing(false); setScanning(false); }}
        className="press w-full rounded-2xl border py-2.5 text-[0.8125rem] font-extrabold"
        style={stocking
      ...
      >
        <span className="inline-flex items-center gap-1.5">
          {stocking ? <><X size={14} /> Close</> : <><Sparkles size={14} /> What’s in my kitchen</>}
        </span>
      </button>
      {stocking && <Card className="!px-0"><KitchenInventory onDone={() => setStocking(false)} /></Card>}
      {adding && <PantryAddForm />}
      ...
      <PantryChecks app={app} confidenceChecks={confidenceChecks} conflicts={conflicts} />
